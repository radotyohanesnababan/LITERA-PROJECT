    ...
    <body class="body">
        <div>
            ...
            <div class="c-manajemen">
                @foreach ($staffmgmt as $mgmt)
                    <div class="text-center flex flex-col items-center ">
                        <div class="bg-image flex items-center justify-center">
                            <img class="img-size" src="{{ asset('storage/staff/' . $mgmt->foto) }}" alt="Deskripsi Gambar 1"
                                class="w-full h-full object-cover">
                        </div>
                        <div class="bg-capt">
                            <p class="nama">{{ $mgmt->nama }}</p>
                            <p class="jabatan">{{ $mgmt->sub_posisi }}</p>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="py-10 xl:py-10">
            <h1 class="judul text-2xl md:text-4xl lg:text-5xl xl:text-6xl text-center">Tenaga Tutor</h1>
                ...
                <div class="c-tentor">
                    @foreach ($stafftentor as $tentor)
                    <div class="text-center flex flex-col items-center ">
                        <div class="bg-image flex items-center justify-center">
                            <img class="img-size" src="{{ asset('storage/staff/' . $tentor->foto) }}" alt="Deskripsi Gambar 1"
                                class="w-full h-full object-cover">
                        </div>
                        <div class="bg-capt">
                            <p class="nama">{{ $tentor->nama }}</p>
                            <p class="jabatan">{{ $tentor->sub_posisi }}</p>
                        </div>
                    </div>
                    @endforeach
